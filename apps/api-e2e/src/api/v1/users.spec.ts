  ...
  describe('PATCH /api/users/{userId}/profile', () => {
    describe('happy path scenarios', () => {
      [
        ...
        it(`${user.role} should be able to update their own profile`, async () => {
      ...
      [
        ...
        it(`admin should be able to update profile for other ${targetUser.role}`, async () => {
          const newName = `${targetUser.name} Admin Updated`;
          const response = await request(
            'patch',
            `/api/users/${targetUser.id}/profile`,
            {
    ...
    describe('error scenarios', () => {
      ...
      [USERS['USER Carol'], USERS['EDITOR Rachel']].forEach(
        (requestingUser) => {
          [
            ...
            if (targetUser.id !== requestingUser.id) {
              it(`${requestingUser.role} should not be able to update profile of other ${targetUser.role}`, async () => {
                const response = await request(
                  'patch',
                  `/api/users/${targetUser.id}/profile`,
                  {
      ...
      it('should return 404 when user does not exist', async () => {
        const response = await request('patch', `/api/users/99999/profile`, {
